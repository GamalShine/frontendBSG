  ...
  const loadLaporanKeuangan = async () => {
    try {
      ...
      if (response.success && response.data) {
          ...
          try {
            ...
            if (Array.isArray(parsedImages)) {
              processedImages = parsedImages.map(img => ({
                ...
                url: img.url || `${envConfig.BASE_URL}/uploads/laporan-keuangan/temp_${img.id}.jpg`,
        ...
        setFormData({
          tanggal_laporan: laporanData.tanggal_laporan ? new Date(laporanData.tanggal_laporan).toISOString().split('T')[0] : '',
          isi_laporan: editorContent,
          images: processedImages
        });
        
        console.log('🔍 Set form data with tanggal_laporan:', laporanData.tanggal_laporan ? new Date(laporanData.tanggal_laporan).toISOString().split('T')[0] : '');
      } else {
          ...
          const newImages = selectedImages.map((img, index) => {
            ...
            if (uploadedFile) {
              return {
                ...
                url: `${envConfig.BASE_URL}${uploadedFile.url}`,
              ...
              return {
                ...
                id: img.id,
                name: `laporan_${img.id}.jpg`,
                url: `${envConfig.BASE_URL}/uploads/laporan-keuangan/temp_${img.id}.jpg`,
                    ...
                    {formData.images.map((image, index) => (
                      <div key={index} className="relative">
                        <img
                          ...
                          onClick={() => insertImage(image.url, image.id)}
                        />
