...
const AdminDataTargetEdit = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await dataTargetService.getById(id);
        if (res?.success && res.data) {
          const it = res.data;
          setForm({
            nama_target: it.nama_target || '',
            target_nominal: String(Number(it.target_nominal || 0))
          });
        } else {
          toast.error(res?.message || 'Gagal memuat data');
        }
      } catch (err) {
        console.error('Load DataTarget error:', err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await dataTargetService.update(id, payload);
      if (res?.success) {
        toast.success('Data Target berhasil diperbarui');
        navigate('/admin/marketing/data-target');
      } else {
        toast.error(res?.message || 'Gagal memperbarui Data Target');
      }
    } catch (err) {
      console.error('Update DataTarget error:', err);
      toast.error(err?.response?.data?.message || err.message || 'Terjadi kesalahan');
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="bg-red-800 text-white p-4">
        <h1 className="text-xl font-bold">Edit Data Target</h1>
