...
import { 
  ...
  ChevronDown,
  ChevronRight,
  Plus, Edit, Trash2, Heart, Baby, PhoneCall, Wallet
} from 'lucide-react';
...
const OwnerDataInvestor = () => {
  ...
  useEffect(() => {
    // Set semua outlet terbuka secara default saat daftar outlet berubah
    const initial = {};
  ...
  const fetchDataInvestor = async () => {
    try {
      setLoading(true);
      const response = await ownerDataInvestorService.getAllDataInvestor();
      if (response.success) {
  ...
  // Parse lampiran dari kolom TEXT: bisa string JSON atau array
    ...
    try {
      ...
      return val
        .map((it) => {
          ...
          // object meta dari backend { url, name, mime, size, stored_name, uploaded_at }
          return {
            url: it.url || '#',
  ...
  const fetchOutlets = async () => {
    try {
      const response = await ownerDataInvestorService.getUniqueOutlets();
      if (response.success) {
...
export default OwnerDataInvestor;
