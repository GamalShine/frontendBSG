...
const OwnerTrainingList = () => {``
  ...
  const [stats, setStats] = useState({
    totalUsers: 0,
    trainingDasarCompleted: 0,
    trainingLeadershipCompleted: 0,
    trainingSkillCompleted: 0,
    trainingLanjutanCompleted: 0
  })
  ...
  const loadTrainings = async () => {
    try {
      ...
      const response = await trainingService.getOwnerTrainings(params)
      console.log('🔍 Owner Training response:', response)
      ...
      if (response.success) {
        ...
        console.error('❌ Owner Training response not successful:', response)
        setTrainings([])
        setTotalPages(1)
      }
    } catch (error) {
  ...
  const loadStats = async () => {
    try {
      const response = await trainingService.getOwnerStats()
      if (response.success) {
  ...
  const exportData = async () => {
    try {
      const response = await trainingService.exportOwnerTrainings({
        status: statusFilter,
      })
      ...
      if (response.success) {
        ...
        a.href = url
        a.download = `training-owner-${new Date().toISOString().split('T')[0]}.csv`
        a.click()
...
export default OwnerTrainingList 