...
const OwnerTimMerahBiruEdit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (type === 'merah') {
        response = await timService.updateTimMerahForOwner(id, formData)
      } else {
        response = await timService.updateTimBiruForOwner(id, formData)
      }

      if (response.success) {
        toast.success(`Tim ${type === 'merah' ? 'Merah' : 'Biru'} berhasil diperbarui`)
        navigate(`/owner/sdm/tim-merah-biru`)
      } else {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              Edit Tim {isTimMerah ? 'Merah' : 'Biru'}
            </h1>
            <p className="text-gray-600">
              Form untuk mengubah data tim {isTimMerah ? 'merah' : 'biru'}
            </p>
          </div>
        </div>
      </div>
...
export default OwnerTimMerahBiruEdit
