...
// Catatan: Disamakan polanya dengan OwnerOmsetHarianForm (editor rich text + upload gambar)

const OwnerTargetHarianForm = () => {
  ...
  // ---- Toolbar helpers (disederhanakan) ----
  const saveSelection = () => {
  ...
  const updateUsedInEditor = () => {
    ...
    const usedIds = new Set(matches.map(m => parseInt(m[1])));
  ...
  const handleEditorPaste = (e) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (it.type.indexOf('image') !== -1) {
        ...
        const imageId = Date.now() + Math.floor(Math.random() * 1000);
        setSelectedImages(prev => [...prev, { file, id: imageId }]);
        const reader = new FileReader();
        reader.onload = (ev) => {
  ...
  const getEditorContent = () => {
    ...
    // Konversi tag <img data-image-id="id"> menjadi placeholder [IMG:id]
    const existingImgRegex = /<img[^>]*data-image-id="(\d+)"[^>]*>/g;
    content = content.replace(existingImgRegex, (match, imageId) => `[IMG:${imageId}]`);

    // Hilangkan semua tag HTML lainnya, simpan line break
    content = content
      .replace(/<br\s*\/?>(?=\n?)/gi, '\n')
      .replace(/<div[^>]*>/gi, '\n')
      .replace(/<\/div>/gi, '')
  ...
  const uploadImages = async (images) => {
    ...
    try {
      ...
      // Samakan struktur seperti di Omset: keep id lokal agar match placeholder
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editorContent || editorContent.trim().length < 10) {
      toast.error('Isi target minimal 10 karakter');
      return;
    }

    setSubmitting(true);
    try {
      // Upload semua gambar yang dipilih (mode create)
      const uploadedImages = await uploadImages(selectedImages);

      // Perbaiki kemungkinan double http:// pada url
      const finalImages = (uploadedImages || []).map(img => {
        if (img?.url?.startsWith('http://http://')) {
          return { ...img, url: img.url.replace('http://http://', 'http://') };
        }
        return img;
      });
      ...
      if (res?.success) {
        ...
        navigate('/owner/marketing/data-target');
      } else {
  ...
  return (
    <div className="px-0 py-2 bg-gray-50 min-h-screen">
      ...
      <div className="bg-red-800 text-white p-4 mb-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => navigate('/owner/marketing/data-target')}
              aria-label="Kembali"
          ...
          <button type="submit" form="form-target-harian" className="bg-white border-red-600 text-red-700 hover:bg-red-50 inline-flex items-center gap-2 px-3 py-2 disabled:opacity-50" disabled={submitting || uploading}>
      ...
      <div className="max-w-4xl mx-auto mb-12 px-4">
        <form id="form-target-harian" onSubmit={handleSubmit} className="bg-white shadow-sm border p-6 space-y-5">
          <div>
          ...
          <div className="flex items-center gap-3">
            <button type="button" className="px-4 py-2 border" onClick={() => navigate('/owner/marketing/data-target')}>Batal</button>
          </div>
        </form>
      </div>
    </div>
...
export default OwnerTargetHarianForm;
