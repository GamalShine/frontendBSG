...
const OwnerTargetHarianEdit = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/owner/marketing/data-target/${id}`);
    } catch (err) {
  ...
  return (
    <div className="px-0 py-2 bg-gray-50 min-h-screen">
      ...
      <div className="bg-red-800 text-white p-4 mb-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/owner/marketing/data-target')} aria-label="Kembali" className="inline-flex items-center bg-white/0 text-white hover:text-gray-100">
          ...
          <button type="submit" form="form-owner-target-edit" className="bg-white border-red-600 text-red-700 hover:bg-red-50 inline-flex items-center gap-2 px-3 py-2 disabled:opacity-50" disabled={submitting}>
      ...
      <div className="max-w-4xl mx-auto mb-12 px-4">
        <form id="form-owner-target-edit" onSubmit={handleSubmit} className="bg-white shadow-sm border p-6 space-y-5">
          <div>
          ...
          <div>
            ...
            {(selectedFiles.length > 0) && (
              <p className="text-xs text-gray-500 mt-1">{selectedFiles.length} file dipilih. Akan diupload saat simpan.</p>
            )}
...
export default OwnerTargetHarianEdit;
