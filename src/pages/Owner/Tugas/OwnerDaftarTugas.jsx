...
const OwnerDaftarTugas = () => {
  ...
  const loadTugas = async () => {
    try {
      ...
      console.log('🔍 Owner Tugas response:', response)
      ...
      if (response.success) {
        ...
        console.error('❌ Owner Tugas response not successful:', response)
        setTugas([])
        setTotalPages(1)
      }
    } catch (error) {
  ...
  const loadStats = async () => {
    try {
      ...
      const belum = tugas.filter(t => t.status === 'belum').length
      const proses = tugas.filter(t => t.status === 'proses').length
      const selesai = tugas.filter(t => t.status === 'selesai').length
      const revisi = tugas.filter(t => t.status === 'revisi').length
      
      setStats({ total, belum, proses, selesai, revisi })
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Belum</p>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-red-600">{stats.revisi}</p>
              </div>
            </div>
          </div>
        </div>
                    ...
                    <tr key={tugasItem.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-2">
                          <Link
                            to={`/owner/tugas/${tugasItem.id}`}
                            className="text-blue-600 hover:text-blue-900 p-1"
                          >
                            <Eye className="h-4 w-4" />
                          </Link>
                          <Link
                            to={`/owner/tugas/${tugasItem.id}/edit`}
