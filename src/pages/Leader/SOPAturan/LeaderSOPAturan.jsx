...
const LeaderSOPAturan = () => {
  // State untuk data SOP (struktur collapsible)
  ...
  const isAdminSopTerkait = location.pathname === '/admin/sop-terkait'

  // State khusus admin: kategori dan info divisi
  const [adminCategories, setAdminCategories] = useState([])
  const [adminDivisiInfo, setAdminDivisiInfo] = useState(null)

  // Fetch SOP: admin gunakan /sop/user-divisi, selain itu gunakan /sop/structure
  useEffect(() => {
    const fetchSop = async () => {
      try {
        setLoadingSop(true)
        if (isAdminSopTerkait) {
          const res = await api.get('/sop/user-divisi')
          const cats = Array.isArray(res?.data?.data) ? res.data.data : []
          setAdminCategories(cats)
          setAdminDivisiInfo(res?.data?.divisi_info || null)
          setErrorSop('')
        } else {
        ...
        setErrorSop(err?.response?.data?.message || err?.message || 'Gagal memuat data SOP')
      } finally {
        setLoadingSop(false)
      }
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-red-800 text-white px-4 sm:px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-extrabold tracking-tight">{isAdminSopTerkait ? 'SOP TERKAIT' : 'S.O.P dan Aturan'}</h1>
              {!isAdminSopTerkait && (
                <p className="text-sm text-red-100">Leader - SDM</p>
              )}
              {/* Info divisi disembunyikan sesuai permintaan */}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-0 py-4 sm:py-6">
        {isAdminSopTerkait ? (
          <div className="bg-white rounded-none md:rounded-xl shadow-sm border border-gray-100 w-full overflow-hidden">
            <div className="px-4 py-3 bg-red-800 text-white flex items-center justify-between rounded-t-md">
              <div>
              ...
              <div className="mb-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Cari S.O.P..."
                    aria-label="Cari S.O.P"
                  />
                </div>
              </div>
              {loadingSop ? (
                ...
                <div className="space-y-3">
                  {(adminDivisiInfo ? adminCategories.filter(c => (c.divisi_nama || '').toLowerCase() === (adminDivisiInfo.nama_divisi || '').toLowerCase()) : adminCategories).map((cat) => {
                    const cOpen = !!openSopCat[cat.id]
                    const stepCount = cat.steps?.length || 0
                    return (
                      <div key={`admin-sop-cat-${cat.id}`} className="border rounded-md">
                        <button
                          type="button"
                        ...
                        >
                          ...
                          <svg className={`h-4 w-4 text-gray-500 transition-transform ${cOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.08 1.04l-4.25 4.25a.75.75 0 01-1.06 0L5.21 8.27a.75.75 0 01.02-1.06z" clipRule="evenodd"/></svg>
                        </button>
                        {cOpen && (
                          <div className="px-3 pb-3">
                            {stepCount > 0 ? (
                              <ul className="mt-1 list-disc list-inside space-y-1 text-sm text-gray-700">
                                {cat.steps.map((step) => (
                                  <li key={`admin-sop-step-${step.id}`}>{step.judul_procedure}</li>
                                ))}
                              </ul>
                            ) : (
                      ...
                      return (
                        <div key={`sop-div-${div.id}`} className="border rounded-md bg-white">
                          ...
                          >
                            ...
                            <svg className={`h-4 w-4 text-gray-500 transition-transform ${dOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.08 1.04l-4.25 4.25a.75.75 0 01-1.06 0L5.21 8.27a.75.75 0 01.02-1.06z" clipRule="evenodd"/></svg>
                          </button>
                          {dOpen && (
                            <div className="px-3 pb-3">
                              {catCount > 0 ? (
...
// Komponen terpisah untuk daftar aturan (hanya untuk non-admin sop-terkait)
const AturanList = () => {
      ...
      {loadingAturan ? (
        ...
        <div className="space-y-3">
          {aturan.map((it) => {
            ...
            const dateValid = it?.tanggal_aturan && !isNaN(Date.parse(it.tanggal_aturan))
            return (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500">
                      {dateValid ? new Date(it.tanggal_aturan).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'} • {it.user_nama || 'Unknown'}
                    </div>
                    {it.isi_aturan && (
...
export default LeaderSOPAturan
